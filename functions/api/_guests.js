...
export async function readGuestPayload(request){let body;try{body=await request.json()}catch{throw new ApiError('请求内容必须是 JSON')};const alias=text(body?.alias,80);if(!alias)throw new ApiError('请填写嘉宾别名');const avatar=text(body?.avatar,100000);if(avatar&&avatarByteLength(avatar)>MAX_AVATAR_BYTES)throw new ApiError('头像不能超过 64KB');return{alias,desc:text(body?.desc,2000),avatar,tags:text(body?.tags,500),quote:text(body?.quote,500)}}
